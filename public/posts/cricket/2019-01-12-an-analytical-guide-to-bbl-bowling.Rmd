...
Ben Laughlin dominates the list with 92 wickets, with the likes of Tye and Abbot gaining podium positions. The 'Wild Thing' rounds out the top 10. However, wickets are not the only things that matter, the amount of runs conceded while taking wickets is also important, therefore things such as average, strike rates, and economy are also important to consider.
...
We see Sodhi who tops the average and strike rate, Malinga who tops the economy, and Khan who has the third best average and second best economy. However, as Ish has only bowled in three innings, if we set a minimum 10 innings, we start to see only one different bowler enter the fray:

``` {r}
paretoCareerBowl <- psel(statBowl %>% filter(Innings > 9) %>% 
                         select(Bowling,Average,Economy,StrikeRate), 
                         low(Average)*low(Economy)*low(StrikeRate))
paretoCareerBowl %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = F, position = "center")
```
Three players have Pareto-Optimal bowling statistics, with Jake Reed joining the list with the superior strike rate.
