...
BBL08 has reached the halfway mark, so let's take a look back on the past four weeks of action to see what has unfolded so far. There has been much discussion around social media about whether lengthening the BBL season has led to a worse competition.
...
datatable(seasonBat %>% 
            mutate_at(vars(StrikeRate, Average), funs(round(., 2))),
            caption = 'Big Bash Season Comparison',
                      options = list(dom = 't'))
```
